...
export function ShareButtons({ shareUrl, shareText, cardId, accentColor = "#facc15" }: ShareButtonsProps) {
  ...
  const captureCard = async (): Promise<Blob | null> => {
    ...
    try {
      ...
      const canvas = await html2canvas(cardEl, {
        ...
        useCORS: true,
        logging: false,
        // Ignore canvas elements (flame/confetti animations) that cause issues
  ...
  const saveCard = async () => {
    ...
    try {
      ...
      if (!blob) {
        alert("Couldn't capture the card. Try taking a screenshot instead!");
        return;
      }
    ...
    <div className="w-full max-w-[480px] flex flex-col items-center gap-5">
      ...
      <div className="flex gap-3 w-full">
        <button
          onClick={saveCard}
          disabled={saving}
          className="flex-1 py-3.5 rounded-xl bg-gradient-to-br from-[var(--accent)] to-[var(--accent-dark)] text-[#09090b] font-['Outfit'] font-bold text-[0.92rem] cursor-pointer transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
          style={{
            "--accent": accentColor,
